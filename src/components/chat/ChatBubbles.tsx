...
const Bubble = ({ children, justification, direction, gap }: Children & {
  justification: "start" | "end"
  direction?: "row" | "row-reverse",
  gap: { [x:string]: string }
}) => {
  return (
    <Flex flexDirection={direction || "row"} justify={justification} w="100%" gap={gap}>
      {children}
    </Flex>
  )
}
...
export const UserBubble = ({
  ...
  return (
    <Bubble gap={{"2xs": "10px", md: "24px"}} direction="row-reverse" justification="end">
      <UserChatAvatar imageSrc={user.profileImage} name={`${user.firstName} ${user.lastName}`} />
...
export const AIBubble = ({
  ...
  return (
    <Bubble gap={{"2xs": "10px", md: "16px"}}  direction="row" justification="start">
      <MedAIChatAvatar />
    ...
    <Bubble gap={{"2xs": "10px", md: "16px"}}  direction="row" justification="start">
      ...
      <HStack spacing="2rem" position="relative">
        <Box  
          ...
          position="absolute" 
          left="30px" w={{"2xs": "0.9rem", md: "1.5rem"}} 
